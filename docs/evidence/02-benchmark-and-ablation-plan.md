...
모든 LLM variant는 같은 commit, task, model version, 허용 도구, evidence snapshot과 inclusive `B_run`을 사용한다. `B_run`에는 input/output/cached tokens, planner, 모든 관점, retry, gate, composer가 포함된다.
...
| P1 | Graph + 5관점 DAG + evidence gate, runtime 없음 | 관점·gate |
| P2 | P1 + focused test | test feedback |
| P3 | P2 + always profiler | profiler ceiling |
| **P4** | **P2 + deterministic selective profiler** | **최종 제안** |
| P4-NG | P4에서 evidence gate 제거 | gate 효과 |
| C1 | P4와 같은 router/gate/tools/budget, 5관점 sequential fixed chain | DAG 병렬·조건부 관점 효과 |
...
- P4 초기 allocation 후보: planner 10%, 관점 합계 50%, merger/gate 20%, composer 20%
...
8. regression으로 version 선택
9. frozen version을 final temporal/OOD에 한 번 실행
...
P3↔P4는 gate-on 상태의 router 차이, P4↔P4-NG는 selective-router 상태의 gate 차이만 추정한다. 완전한 `router × gate` 상호작용은 no-gate always variant가 없으므로 주장하지 않는다.
...
P4는 등록 후보 안에서 다음을 모두 만족할 때만 Pareto 우월로 보고한다.
...
오류: retrieval, graph resolution, pruning, reasoning, unsupported claim, wrong severity, invalid workload, under/overtrigger, instability, merge loss, report omission, protocol exposure.
...
모든 비교값은 미측정이다. 이 문서는 실험 계약이며 외부 숫자를 결과처럼 채우지 않는다.
